...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            distance = self.getPositionDistribution(gameState, position)
            for pos in distance:
                probability[pos] += distance[pos] * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for position in self.legalPositions:
            for _ in range(particles_per_position):
                self.particles.append(position)
        leftover_particles = self.numParticles - len(self.particles)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            weight = self.getObservationProb(
                observation, pacman_position, particle, jail_position
            )
    ...
    def elapseTime(self, gameState):
        ...
        for old_pos in self.particles:
            ...
            new_pos = new_pos_distribution.sample()
