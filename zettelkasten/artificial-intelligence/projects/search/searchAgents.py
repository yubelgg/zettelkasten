...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        # check if it has reached all four corners
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            if not hitsWall:
                if next_state in self.corners and next_state not in state[1]:
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    "*** YOUR CODE HERE ***"
    # can't fill as we go, remove as we go
    total_dist: int = 0
    curr_state: tuple[int, int] = state[0]
    ...
    while not_visited:
        min_dist = util.manhattanDistance(curr_state, not_visited[0])
        min_corner: tuple[int, int] = not_visited[0]

        for corner in not_visited:
            dist = util.manhattanDistance(curr_state, corner)
            if dist < min_dist:
                min_dist = dist
                min_corner = corner

        total_dist += min_dist
        curr_state = min_corner
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    "*** YOUR CODE HERE ***"
    # find the highest heuristic that is admissable
    heuristic: int = 0
    ...
    for food in foodList:
        dist = mazeDistance(position, food, start_state)
        if dist > heuristic:
            heuristic = dist
